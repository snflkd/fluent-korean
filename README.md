...
- 생각보다 원하는 만큼 동작하지 않을 수 있습니다. 주어진 지시가 다양할수록, 작업이 길게 진행되었을수록, Priming을 유발하는 텍스트가 많을수록 그런 경향이 강해집니다. 이 경우에는 지침 자체를 수정하기보다는, 이를 효과적으로 수행하도록 하네스를 구성 및 최적화해두시는 것을 추천드립니다. (저는 output-style을 기본으로 쓰되, 주요 산출물에서 문체가 잘 안 지켜지면 Skill처럼 활용하여 개선하고, 하네스에서는 결과물을 도출하기 전에 적대적 검증을 수행하도록 세팅해두었습니다.)
